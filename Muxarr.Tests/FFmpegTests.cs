...
public class FFmpegTests
{
    ...
    public void BuildArguments_NullFieldsOnTrack_EmitsNothingForThatTrack()
    {
        var tracks = new List<TrackPlan>
        {
            new() { Index = 2, Type = MediaTrackType.Subtitles }
        };
    ...
    public void BuildArguments_SetsTitleAndLanguage()
    {
        ...
        // -map refers to the input stream (Index=1), -metadata and
    ...
    public void BuildArguments_MultipleTracks_EmitsPerStreamOptions()
    {
        ...
        {
            new() { Index = 0, Type = MediaTrackType.Video },
            new() { Index = 1, Type = MediaTrackType.Audio, LanguageCode = "eng", IsDefault = true },
            new() { Index = 2, Type = MediaTrackType.Subtitles, LanguageCode = "dut", Name = "Dutch" }
        };
    ...
    public void BuildArguments_ReorderedTracks_UsesOutputIndicesForMetadata()
    {
        ...
        var tracks = new List<TrackPlan>
        {
            new() { Index = 2, Type = MediaTrackType.Audio, Name = "First out" },
            new() { Index = 0, Type = MediaTrackType.Video },
            new() { Index = 3, Type = MediaTrackType.Subtitles, Name = "Third out" }
        };
    ...
    public async Task RemuxFile_ThrowsOnSameInputOutputPath()
    {
        await Assert.ThrowsExactlyAsync<ArgumentException>(async () =>
            await FFmpeg.Remux("/same.mp4", "/same.mp4", TestPlan.Of(new List<TrackPlan>())));
    }
    ...
    public void BuildArguments_FaststartFalse_DoesNotAppendFaststartFlag()
    {
        var args = FFmpeg.BuildRemuxArguments("/in.mp4", "/out.muxtmp",
            TestPlan.Of(new List<TrackPlan>(), false));
    ...
    {
        var args = FFmpeg.BuildRemuxArguments("/in.mp4", "/out.muxtmp",
            TestPlan.Of(new List<TrackPlan>(), true));
    ...
    public void BuildArguments_MovMuxer_EmitsDashFMov()
    {
        var args = FFmpeg.BuildRemuxArguments("/in.mov", "/out.muxtmp",
            TestPlan.Of(new List<TrackPlan>()), "mov");
    ...
    /// </summary>
    private async Task<List<TrackPlan>> BuildAllTracks(Action<List<TrackPlan>>? mutate = null)
    {
    ...
    public async Task RemuxFile_SetsTitleOnAudioTrack()
    {
        ...
        {
            var audio = ts.First(t => t.Index == 1);
    ...
    public async Task RemuxFile_SetsCommentaryFlagOnMp4()
    {
        ...
        var tracks = await BuildAllTracks(ts =>
        {
            ts.First(t => t.Index == 1).IsCommentary = true;
            ts.First(t => t.Index == 2).IsCommentary = false;
        });
    ...
    public async Task RemuxFile_SetsDefaultAndForcedFlagsOnMp4()
    {
        ...
        var tracks = await BuildAllTracks(ts =>
        {
            ts.First(t => t.Index == 1).IsDefault = false;
            ts.First(t => t.Index == 2).IsDefault = true;
            ts.First(t => t.Index == 3).IsForced = true;
        });
    ...
    public async Task RemuxFile_DispositionRoundTripsThroughSetFileDataFromFFprobe()
    {
        ...
        var tracks = await BuildAllTracks(ts =>
        {
            ...
            ts.First(t => t.Index == 3).IsHearingImpaired = true;
        });
    ...
    {
        var output = _workingCopy + ".muxtmp";
        var tracks = await BuildAllTracks(ts => { ts.First(t => t.Index == 2).LanguageCode = "fre"; });
    ...
    public async Task RemuxFile_KeepsEveryStream()
    {
        ...
        var tracks = await BuildAllTracks(ts => { ts.First(t => t.Index == 1).Name = "Touched"; });
    ...
    public async Task RemuxFile_RoundTrip_ScannerSeesNewTitle()
    {
        ...
        var output = _workingCopy + ".muxtmp";
        var tracks = await BuildAllTracks(ts => { ts.First(t => t.Index == 1).Name = "Round Trip Title"; });
